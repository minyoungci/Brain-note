...
- 단, 이는 "deep이 무용하다"가 아니라, 더 어려운 task(progression)나 더 나은 표현(SSL)에서 비로소
  가치를 입증할 수 있다는 연구 기회의 정의이기도 하다(plant·minyoung3의 존재 근거).
...
- ⚠️ **single-cohort 함정**: APOE·MoCA=NACC only / MMSE=ADNI 없음 / sex NaN=A4·ADNI(→`clin_sex_raw`).
...
minyoung2(EXP01 cross-sectional 프로토콜·성숙)가 척추이며, plant(종단 확장)와 minyoung3(SSL 표현)가
그 프로토콜을 각각 시간축·표현학습으로 확장한다. minyoungi가 데이터·문헌·ROI QC를 공급하고, minyoung4는 휴면.
