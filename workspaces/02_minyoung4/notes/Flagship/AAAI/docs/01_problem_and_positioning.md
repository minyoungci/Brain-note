...
- 강한(현 증거 기준): `we show that effective rank decouples from transfer under joint dense+global objective balancing — so RankMe-style rank-maximization selection picks the worst checkpoint (regret 0.19, CI-separated) — and that a pre-registered battery of label-free spectral criteria fails to locate the interior transfer optimum on a symmetric single-task grid (a genuine open problem, not a solved selector); plus a budget/protocol-adaptive transfer method — for large-scale (FOMO300K) single-checkpoint 3D brain MRI foundation models. (A dense/off-center-optimum task test and external multi-site validation are the load-bearing next steps; not yet claimed.)`

## AAAI 기술적 기여 (TC1/TC2/TC3) — 증거 스트림 C1/C2/C3

> **Positive technical novelty (headline)** = backbone(SparK 인용)이 아니라:
> - **TC2 (headline = cautionary finding + open problem) — objective-balance와 rank–transfer decoupling**:
>   FINDING(견고) — dense+global 결합에선 effective rank가 transfer와 *분리*(rank 단조↓ vs transfer inverted-U)되어
>   **RankMe식 rank-최대화 선택이 최악 체크포인트(wg0)를 고름**(regret 0.194, CI-분리). METHOD(NO-GO, 2026-07-01
>   5점 검정) — 사전등록 라벨-프리 기준(α-ReQ/EVR/silhouette) 어느 것도 내부 정점을 못 짚음; α-ReQ의 3점 peak는
>   coarse-grid 아티팩트, 게다가 최적=grid 중점이라 "중점 찍기" trivial default에도 짐 → **robust label-free selector
>   부재 = open problem**. RankMe delta: 그들=label-free *model 순위* / 우리=objective-balance에서 rank가 *실패함*을
>   통계적으로 입증(경고). **구원 경로(사전등록)**: dense/seg task(off-center 최적점 후보)로 중점-triviality를 깨고
>   held-out unlabeled·LOTO·baseline격파 하에서만 positive selector 재주장 — `docs/10` 참조.
...
### Claim C2 (headline = cautionary finding + open problem). Rank–transfer decoupling; label-free selector = NO-GO

```text
FINDING (verified, 5-point grid): dense+global 결합 SSL에선 effective rank가 transfer와 *분리(decouple)*된다.
  rank 단조↓(train RankMe 14.86→12.93→11.65) vs transfer inverted-U(0.599,0.774,0.792,0.768,0.683).
  → RankMe식 rank-최대화 선택은 rank-max인 wg0(0.599)을 골라 *최악을 고른다*: regret 0.194(~4SE, CI-분리).
    (rank류=participation/stable_rank도 동일 오선택.) RankMe에 대한 non-obvious·통계적으로 견고한 경고.
METHOD (NO-GO, 2026-07-01): 사전등록 라벨-프리 기준 {α-ReQ, EVR-top10, silhouette} + uniformity 스크린 →
  argmax-일치 gate 전부 실패. α-ReQ 3점 peak@wg0.5는 coarse-grid 아티팩트(5점서 argmax@wg0.25),
  regret 0.019는 (i) 노이즈(Δr≈0.4SE) (ii) "grid 중점 찍기" trivial default(regret 0)보다 나쁨
  ← 최적 wg0.5가 grid 중점과 겹치는 confound. uniformity(regret 0)는 비후보=사후승격 금지(다중비교).
  → robust label-free selector는 현 증거로 *부재* = open problem.
CRITIC(2026-07-01): 위 NO-GO는 사전등록 gate에 의해 자동 판정. positive selector headline = HARKing/overclaim.
구원(사전등록 필요, docs/10): dense/seg task(off-center 최적점)로 중점-triviality를 깨고,
  held-out UNLABELED에서 지표 계산 + multi-task LOTO + trivial baseline 격파 시에만 positive 재주장.
정직 경계: 내부 interior-optimum task = brain-age n=1. "rank fails" 경고도 inverted-U 존재에 의존
  → C3 외부서 inverted-U 미재현이면 경고 자체 붕괴. 외부검증=[PENDING], 완료형 금지.
```
