...
_갱신: 2026-06-02 · 출처: SCRATCHPAD(Last updated 2026-06-01), prereg(2026-06-01), summary.json_

## 결과 상태: **결과 없음, 설계 단계**
모델링 산출물 일절 없음(`results/`·`configs/` 비어 있음). 아래는 (a) 검증된 설계 결정·feasibility 제약,
(b) 빌드+검증된 label table 통계, (c) prereg의 사전등록 판정기준이다. 모든 c-index/AUROC 수치는 **미측정**.
...
4. **shuffled-label** — leakage probe (c-index≈0.5 나와야 정상).
...
| NACC | session_id=이미지ID | ❌ **불가**(순서X) | ❌ 제외 | 시간 정렬 불가능 |
...
| OASIS | 363 | 317 | 30 | 5.01 | train-pool (30 pos: 너무 적음) |
...
> ⚠️ **수치 불일치 [VERIFY]**: SCRATCHPAD §4 feasibility 텍스트는 "A4 96, total 270"으로 기재하나,
> §5·prereg §4·summary.json은 모두 **A4 98, total 272**. 빌드 산출물(summary.json) 기준이 정합적이며 96/270은 구버전 추정치로 보임.

## 5. 사전등록 판정기준 (prereg §7 — 결과 나오면 적용)
...
- image-full은 shuffled를 이겨야(sanity)하고 mask-only와 비교해 신호 출처 국소화.
...
- cohort-as-shortcut: cohort-ID-only baseline은 LOCO에서 c≈0.5(전이 실패)여야 정상.
