...
> 주의: `foundation_model_design.md` §10 표는 2026-06-30에 현재 best(trig 0.450·men 0.159·d2_probe floor)로 갱신됐고, 초기(Jun 25) full-FT 결론("seg는 from-scratch를 못 넘는다")은 §10 이력란과 이 노트 Wave-E에서 뒤집힌 경위가 함께 기록돼 있다. **두 문서 모두 최신이며, 전체 여정·실패 교훈은 이 노트가 단일 출처.**
...
- ⚠️ **하드 교훈 (내부 ≠ hidden)**: 이 0.942는 내부 LOOCV다. 실제 Synapse hidden validation에 처음 제출했을 때 **AUROC = 0.658**이었다 — n21 full-FT가 21명에 과적합한 것. 그래서 v2_frozen 경로(frozen-encoder + linear head + dwi/adc/flair만)를 만들었고 LOOCV는 다시 0.942지만 **hidden은 아직 미검증**. **작은-n cls의 내부 점수는 hidden 성능의 증거가 아니다** — 이게 이 프로젝트에서 가장 비싸게 배운 교훈이다.
...
| Wave-I | controlled ablation(trig→men 조건) | combo 0.379 | 천장 인과 검증(아래 5번) |
| (별도) | Flagship S3D-Vista adapter F1 | 0.116 (Δ+0.032) | adapter도 best 못 넘음 |
...
5. **천장 인과 검증과 그 한계** (Wave-I): "men 0.159가 정말 thick-slice+n23 *때문*이냐(decoder/모델이 아니라)"를 확인하려고 trigeminal(isotropic n40)을 men 조건으로 깎아봤다. n23+thick6 combo는 **0.379**로 떨어졌다 — 방향(데이터를 깎으면 성능이 준다)은 맞다. **그러나 솔직히 말하면 0.379는 men 실제 0.159와 2.4배 차이가 나고, thick-slice 단독은 오히려 점수를 올렸다(0.432).** 즉 Wave-I는 "데이터(해상도+n)만으로 men을 *완전히* 설명한다"는 sufficiency는 증명하지 못했다. 남는 격차의 주원인은 **병변형(extra-axial blob)·모달(trig는 t2w·men은 flair)·single-modality few-shot**이다. (원래 SUMMARY가 "sufficiency 증명"이라 적었던 건 과장이라 2026-06-30 정정했다 — 자기생성 결론을 검증 없이 믿지 않는다.)

**정직한 결론**: men의 천장은 **n=23 few-shot + extra-axial blob + 8.2배 thick-slice + single-flair**의 복합 때문에 낮다(현실 상한 ~0.2-0.3 [VERIFY]). 0.159는 우리 foundation/레시피의 결함이라기보다 **challenge가 준 데이터의 본질적 한계**에 가깝다(decoder 재설계·adapter·cascade 모두 못 넘었다 — Wave-D/E/G/I + Flagship adapter). 단 "데이터만으로 100% 설명"은 Wave-I로도 입증 못 했고, 병변형·모달 차이가 격차의 일부다. trigeminal(0.5mm isotropic, t2w)이 0.45인데 men(6.5mm, flair)이 0.16인 격차는 해상도·n·병변형·모달이 함께 가른다.
...
|---|---|---|
| brain age | r 0.947 | +0.037 (frozen 0.792 vs random 0.137, **Δ+0.656**) |
| infarct | AUROC 0.942 ⚠️ | +0.346 (단 **실제 hidden=0.658**, Part 4.2) |
| polymicro | 0.986 | confound (frozen 0.957 vs random 0.608, Δ+0.349) |

> frozen-probe floor는 최신 d2_probe(2026-06-29) 기준. objective-balance inverted-U: brainage wg0→0.5→1 = 0.599→0.792→0.683(wg0.5 peak).

### 남은 작업 (제출 트랙)
- **S6**: 제출 컨테이너 (Apptainer, base torch2.11/cuda12.6) — task1 v2_frozen + task2 full-FT + task4 frozen-lowlr + task3/5 finetune + task6/7 embedding 번들. B200(sm_100)서 build·GPU테스트 불가 → 로직 B200검증·동일코드·validator --no-gpu.
- **S5**: Task6/7 embedding 컨테이너 + fairness (그룹 메타 부재로 로컬 fairness 보류).
- **S7**: 마지막 test 제출 1회(2회 중 1회 사용=0.658, best-of-all selection이라 무손실). best-selection 단위(task-wise vs overall) Submission.md과 확인.
- task5 frozen-mitigation 점검(task1형 cls n48 과적합 리스크).
