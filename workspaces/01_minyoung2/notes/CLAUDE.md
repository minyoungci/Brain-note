...
4. **위험·모니터** — W1~17 + monitor.py(collapse/bf16/teacher STOP/WARN) → `docs/06_risk_register.md`
...
8. **Downstream 일반화(필수)** — downstream n 작아 **내부/Validator 점수 ≠ hidden test**(Task1 0.94→0.658 입증). finetune·제출 전 일반화 우선(frozen/정규화/선택과적합 차단). cls(T1·T5) 최고위험 → `docs/09_downstream_generalization.md` (W17)

> 핵심은 *경로*가 아니라 "**설계·데이터·규칙·위험·상태·shortcut통제·일반화를 실제 훈련/평가 전 반드시 읽고 확인**"이다.
