...
Status: first Results-section draft aligned to `docs/JAMIA_ABSTRACT_BASELINE.md` baseline v2.
...
The residual checklist failures were not random. All three checklist over-claims occurred on the same incremental-value case, indicating that a global checklist instruction reduced broad over-claiming but did not reliably enforce a case-specific claim ceiling. Human-corrected recurrence review supported this localization: the checklist recurrence target was adjudicated as an E2 safety over-claim in 3/3 repeats, while separate rule/judge spot-checks reclassified several calibrated negations as completeness gaps rather than safety failures.
...
The empirical question is whether a replication-aware baseline can recover these traps using a disjoint held-out replication signal. Across all 214 hard-artifact-stratum cases, the replication-rule baseline detected 148/206 artifact-labeled cases (recall 0.718; 95% CI, 0.653-0.775) at precision 0.987 (148/150; 95% CI, 0.953-0.996). On the 98 naturally occurring non-planted hard traps, it detected 42/98 (42.9%; 95% CI, 33.5%-52.7%) at precision 1.000 (42/42; 95% CI, 91.6%-100%). Thus, replication-aware rules recover part, but not all, of the designed hard stratum; the remaining missed natural traps define the unsaturated portion of the benchmark.
...
The results support three bounded claims. First, open-ended medical research-agent outputs can over-claim structured biomedical artifacts, and checklist prompting reduces but does not eliminate those over-claims in the 30-case pilot. Second, the scaled benchmark creates a replication-defined hard stratum in which discovery evidence and replication evidence disagree; a replication-rule baseline recovers only part of the naturally occurring traps. Third, direct yes/no LLM evaluation on the scaled benchmark does not show over-claiming, but rather near-uniform rejection. Together, these findings motivate ClaimTrap-AD as an evaluation benchmark for claim calibration, not as evidence that any current controller or LLM agent solves the problem.
