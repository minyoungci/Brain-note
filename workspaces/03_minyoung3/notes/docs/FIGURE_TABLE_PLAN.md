...
### Figure 2 — Dual-view design & gold-leak correction  [Contribution 2 — Leakage control]
```
 ...
 verification prompt                      generation_view ──► agent (generic/checklist/controller)
   ...
   = ANSWER-AWARE agent (deprecated)      → leak-free blind baseline; mechanism is task-agnostic
```

### Figure 3 — Claim Safety Controller (Algorithm 1) + trace  [Contribution 3 — TECHNICAL CORE]
```
...
### Figure 4 — Main safety/completeness results (n=3, 90 outputs/arm)  [Contribution 4 — empirical finding: trade-off]
```
