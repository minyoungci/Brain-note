...
Working draft — 2026-06-10. Numbers are from this repository's runs (macro AUC, 3 seeds
unless stated). Main multi-cohort claims are validated under strict LOCO on AJU/OASIS/NACC;
the ResNet backbone comparison is AJU-only (2 seeds) and reported as an honest limitation.

## Abstract

We study image-only ROI-grounded visual question answering (VQA) on 3D T1w brain MRI,
where a model answers normative anatomical-evidence questions (low hippocampal volume,
medial-temporal atrophy, ventricular enlargement, low hippocampus-to-ventricle ratio) from
image tensors and a question id alone, with all clinical, cohort and ROI metadata excluded.
On a shortcut-controlled, subject-level leave-one-cohort-out (LOCO) benchmark (clinical-
context AUC ~ chance), we ask: *what form of ROI conditioning actually improves
fine-grained 3D MRI VQA, and when?* We introduce a **parameter-efficient anatomy-prior-
guided question router** that dispatches each question to a dedicated high-resolution ROI
expert via a learned gate, weakly regularized toward an anatomical prior, and we compare it
against single-view fusion, naive multi-crop concatenation, learned 3D evidence
localization (weak ROI-mask supervision), and cross-ROI relational reasoning, with a full
ablation across encoder regimes and backbones. Findings: (1) the learned router robustly
beats single-view and multi-crop baselines across all three held-out cohorts (AJU/OASIS/
NACC) under strict LOCO, bootstrap-significant in five of six cohort-router cells, with
gains concentrated in fine medial-temporal questions (MTL +0.12, hippocampal +0.10 vs
multi-crop), and it exceeds a hard anatomical router while the anatomy prior acts as a
weak regularizer (not a hard-coded lookup: a prior-free router still beats single-view and
its gate is data-driven). (2) Learned soft localization and cross-ROI relational reasoning
do not help — the benefit is from routing to dedicated high-resolution ROI evidence, not
from localizing within a global volume or relating pooled features. (3) The conditioning
benefit is **regime-dependent**: it is invisible with from-scratch or frozen-contrastive
encoders and emerges only on a fine-tuned 3D-SSL base, and the routing gain monotonically
vanishes as the backbone scales (router vs multi-crop: +0.070 -> +0.006 -> -0.005 for a
0.35M compact encoder, a 14M ResNet-10 and a 33M ResNet-18). Yet the compact 0.35M routed
model is the best configuration overall, outperforming both the 14M and 33M ResNet
backbones — so the contribution is parameter-efficient anatomical conditioning (a tiny
encoder made competitive with 40-90x larger 3D backbones) rather than a universal accuracy
gain stacked on any encoder. We also show that standard
...
### 4.6 Backbone comparison (does routing need a toy encoder?)

We replace the compact 4-layer encoder (~0.35M params) with a real 3D backbone (MONAI
ResNet-10, ~14M params) for every expert, contrastive-pretrained with AJU excluded
(strict LOCO) and fine-tuned identically. AJU LOCO, 2 seeds, macro AUC:

| backbone | params | B1 | B2 | router (lambda=0.3) | router vs B2 |
|---|---:|---:|---:|---:|---:|
| compact | 0.35M | 0.837 | 0.812 | **0.882** | +0.070 |
| ResNet-10 (pretrained+ft) | 14M | 0.819 | 0.838 | 0.844 | +0.006 |
| ResNet-18 (pretrained+ft) | 33M | 0.869 | 0.865 | 0.860 | -0.005 |

Two clean trends. (i) The routing advantage monotonically vanishes as the backbone scales
(router vs B2: +0.070 -> +0.006 -> -0.005): a larger backbone already captures much of the
per-question relevant anatomy, so explicit anatomical routing adds little and, on ResNet-18,
nothing. (ii) Despite this, the compact 0.35M routed model (0.882) is the best overall
configuration, exceeding even the 33M ResNet-18 (best single-view 0.869, B2 0.865, router
0.860). The contribution is therefore parameter-efficient anatomical conditioning: routing
lets a tiny encoder outperform 40-90x larger 3D backbones, rather than a universal accuracy
gain stacked on any encoder. (ResNet rows are 2 seeds, AJU only.)
