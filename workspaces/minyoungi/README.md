...
> **목적:** 지원 워크스페이스(문헌·clinical 이해·ROI QC)의 현황 요약  ·  **출처:** `/home/vlm/minyoungi` (커밋 2bfa860)  ·  **갱신:** 2026-06-02
...
학습·평가 실험은 `/home/vlm/minyoung2`, `/home/vlm/minyoung4`에서 수행한다 (README/AGENTS.md 명시). ✅
단, 이 워크스페이스 안에도 `experiments/voxelwise_feature_learning_v1`이 존재하며 Gate04~05 진단 실험 산출물이 누적되어 있다 — README의 "실험 코드는 두지 않는다" 원칙과 현황이 부분적으로 어긋남. 🟡

## 현재 활동 (2026-05-28 ~ 06-02)

- **Gate05b ROI/structured-language supervision 설계 + NACC 실패 audit** (최근 커밋 흐름 전체). 🟡
  - b1(global ROI-cos)은 ADNI/KDRC를 개선하나 **NACC는 회귀** → 라벨 `image-baseline-partial-pass with NACC regression`. ❌ (representation-readiness-pass 아님, vlm-scaling-ready 아님)
...
3. leave-one-consortium-out로 CDR AUC 0.9가 site 누수인지 검증 (random split은 site 누수).

## 한 줄 리스크

이 워크스페이스가 의존해 온 ROI 데이터는 공식적으로 `BLOCKED_PROVISIONAL`이고 사람 시각 승인 전이므로, ROI 기반 모든 정량 결론은 "검증됨"이 아니라 "후보(provisional)"로 강등해야 한다. ❌
