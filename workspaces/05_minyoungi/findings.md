...
> **목적:** clinical 이해·Gate05b·ROI QC에서 검증된 발견 기록  ·  **출처:** `/home/vlm/minyoungi` daily/VALIDATION_LOG/roi_qc (커밋 2bfa860)  ·  **갱신:** 2026-06-02

범례: ✅ 확정 / ❌ 반증 / 🟡 잠정 / `[VERIFY]` 미검증
...
- **교훈**: "정말 그게 맞는가"라는 의심이 정확했다. 결측처럼 보이는 것의 1차 용의자는 데이터가 아니라 join/normalize 버그다.

### 1.3 Single-cohort 함정 (적대적 비평이 포착)
...
- **AUC 0.9를 "유효 타깃" 근거로 쓰지 말 것** — random split은 site 누수. leave-one-consortium-out 전에는 낙관치. 🟡
...
- ROI-cos 가중치를 낮추거나 CE를 높여도 NACC 회복 안 됨 → b0가 여전히 NACC 최선. ✅
...
- **해석**: b1 global ROI-cos loss가 특정 ROI-stat 타깃의 alignment를 손상시킨다. → b2는 global row-text alignment를 피하고 위 5개 타깃을 per-target 모니터링해야 한다. 🟡 (진단일 뿐, anatomical causality나 VLM readiness 증명 아님)
...
- 그러나 brain-only 입력 = original 입력 (final_tensor가 이미 skull-strip/zero-background) → "raw 배경 강도 shortcut" 주장은 약화. 병목은 **brain 내부 disease-relevant 3D feature를 약하게/오정렬해서 사용하는 것**. ✅
...
- **의미**: ROI-language/VLM 모델이 이 baseline을 넘지 못하면 MRI representation이 아니라 deterministic ROI status/QC metadata 재포장일 가능성. 모든 후속 VLM claim의 게이트.

---

## 3. ROI QC: BLOCKED_PROVISIONAL — 가장 뼈아픈 발견

### 3.1 사용해 온 ROI는 공식적으로 "잠정(BLOCKED)"이었다
...
```
→ 03/05/06/컨소시엄 노트북이 의존한 option_b ROI는 **시각 승인 전 candidate**다. 결과가 틀렸다는 의미가 아니라, 모든 ROI 기반 결론을 "검증됨"→"후보"로 강등해야 한다. ❌ (검증된 데이터 아님)

### 3.2 centroid 좌표계 불일치

- summary JSON의 `centroid_voxel`은 256-conformed 좌표라 final_tensor 그리드와 **~31 voxel 어긋남**. ✅
- 큐브 크롭 중심은 `expected_centroid_final_voxel` 또는 마스크 재계산값을 써야 한다. (`roi_tools.centroid()`는 마스크 재계산이라 기존 오버레이는 정상.) ✅
...
- **3-게이트 weakest-link**: ① numeric transfer ② auto-geometry ③ **vision QC(미완)**. Gate2 PASS ⇏ Gate3 PASS. 올바르게 위치하고 대칭이며 single-component인 boundary error는 게이트1·2가 잡지 못한다. 🟡
...
2. **생성 ≠ 검증**: 구조가 목표와 1:1로 맞는 노트북 5개 중 4개가 현재 데이터에서 깨져 있었다(sex 타입 혼합, FastSurfer 경로 누락, eTIV 부재). 구조가 맞다고 동작하는 것은 아니다.
3. **검증 ≠ 적대적 검증**: 단독 작업이 잡지 못한 두 가지(BLOCKED_PROVISIONAL 플래그, centroid 좌표계 불일치)를 멀티에이전트 적대적 비평이 포착했다.
4. **자기일관성 ≠ 정확성**: `auto_verdict≈PASS`는 전송 자기일관성일 뿐, FastSurfer 원본 분할 오류는 잡지 못함 → vision 필수.
