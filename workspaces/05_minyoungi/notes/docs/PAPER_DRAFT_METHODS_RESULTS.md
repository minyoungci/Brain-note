...
**Status: full-prose expansion of §3–§8 for AAAI framing. No experiments / no LLM calls / no code changes.
All numbers verified against committed artifacts. All literature and "first/novel" claims are [VERIFY] pending
literature-scout. Controller FROZEN at v4. Companion: PAPER_DRAFT_CLAIMTRAP_AD.md (abstract/intro/contributions),
FIGURE_TABLE_PLAN.md (figures/tables).**
...
**safety–completeness trade-off** that we quantify in §7. [VERIFY: novelty of this formulation pending
literature-scout.]
...
zero gold tokens reach any agent prompt. The ClaimTrap30 dual-view blind path is thus the first valid baseline in
this project; the deprecated 5-case numbers are reported, if at all, only as exploratory context. Crucially, the
...
## 4. Hybrid Evaluator
Scoring calibrated scientific prose is itself error-prone, so we combine three stages. (1) A **rule-based
...
position, verbosity, and self-enhancement biases [VERIFY: MT-Bench, arXiv:2306.05685], which we mitigate by
using a non-self judge and never treating the judge as the sole authority. (3) A **human spot-check** locks
...
prompt that requests caution and an *algorithm that controls claim generation*.
...
free-text clinical notes (§8). No learning (DPO/SFT) is used. [VERIFY: AAAI reproducibility-checklist items.]

## 7. Results
### RQ1 — Do generic medical research agents over-claim?
Yes, and systematically. The generic agent over-claims on **19/90** outputs and hard-fails on **14/90**
